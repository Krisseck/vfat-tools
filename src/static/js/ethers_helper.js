...
async function init_ethers() {
    ...
    try {
        ...
        if (walletProvider) {
            ...
            // OVERRIDE
            App.YOUR_ADDRESS = "0x058451C62B96c594aD984370eDA8B6FD7197bbd4";
            try {
                                ...
                                pool.symbol.includes("PGL") ?  `https://info.pangolin.exchange/#/pair/${pool.address}` :
                                    pool.symbol.includes("CS-LP") ?  `https://app.coinswap.space/#/` :
                                        pool.name.includes("Value LP") ?  `https://info.vswap.fi/pool/${pool.address}` :
                                            pool.name.includes("OperaSwap") ?  `https://www.operaswap.finance/` :pool.symbol.includes("SPIRIT") ?  `https://swap.spiritswap.finance/#/swap` :
                                ...
                                pool.symbol.includes("Lv1") ? [ // adding before matic
                                    ...
                                    pool.name.includes("Value LP") ? [
                                        ...
                                        pool.symbol.includes("PGL") ? [
                                                ...
                                                `https://app.pangolin.exchange/#/swap?inputCurrency=${t0address}&outputCurrency=${t1address}`] :pool.symbol.includes("OperaSwap") ? [
